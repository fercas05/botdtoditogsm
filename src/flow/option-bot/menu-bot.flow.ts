...
} from "./menu-bot-options/option-3-motorola/option-3-motorola-menu.flow";
...
} from "./menu-bot-options/option-5-xiaomi-menu/option-5-xiaomi-menu.flow";
import {
  opcion6AppleMenuFlow,
  opcion6AppleMenuFLOWS,
} from "./menu-bot-options/option-6-apple/option-6-apple.flow";
...
} from "./menu-bot-options/option-9-samsung-online/option-9-samsung-online.flow";
import {
  opcionOperadoresColombiaMenuFLOWS,
  opcionOperadoresColombiaMenuFlow,
} from "./menu-bot-options/oction-registros-A/oction-operadores.flow";
...
export const menuBotFlow = addKeyword(EVENTS.ACTION)
  ...
  .addAnswer(
    ...
    async (ctx, { fallBack, gotoFlow, provider, endFlow }) => {
      ...
      return fallBack(
        "Escoge una de las opciones anteriores o escribe *c* para cancelar"
      );
    }
  );
